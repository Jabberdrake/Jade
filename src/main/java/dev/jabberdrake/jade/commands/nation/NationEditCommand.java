...
public class NationEditCommand {
    ...
    public static int runCommandForName(CommandContext<CommandSourceStack> context) {
        ...
        if (!RealmManager.isUniqueNationName(nameArgument)) {
            player.sendMessage(error("That name (<highlight>" + nameArgument + "<normal>) is already in use!"));
            return Command.SINGLE_SUCCESS;
        }

        focus.getNation().setName(nameArgument);
        focus.getNation().broadcast("Changed <highlight>reference name</highlight> to <highlight><i>" + focus.getNation().getName() + "<normal>!");
        return Command.SINGLE_SUCCESS;
    }
    ...
    public static int runCommandForIcon(CommandContext<CommandSourceStack> context) {
        ...
        focus.getNation().broadcast("Changed <highlight>icon</highlight> to <highlight>" + focus.getNation().getIconAsString() + "<normal>!");
        return Command.SINGLE_SUCCESS;
    }
}
